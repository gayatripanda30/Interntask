...
const Navbar = () => {
  ...
  return (
    <nav className="fixed top-0 z-50 w-full">
      ...
      {/* NAVBAR */}
      <div
        className={`flex items-center justify-between px-6 h-[63px] transition-all duration-300 ${
          scrolled
      ...
      >
        ...
        {/* DESKTOP MENU */}
        <ul className="items-center hidden gap-8 font-bold text-white md:flex">
          {navItems.map((item) => (
            <li key={item.path}>
        ...
        >
          <span
            className={`w-8 h-[3px] bg-white rounded transition-all ${
      ...
      {/* MOBILE DRAWER */}
      <div
        className={`fixed top-0 right-0 h-screen w-[70vw] max-w-[420px] bg-[#04152a] text-white shadow-2xl transition-all duration-300 z-[1500] ${
